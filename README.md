...
- **本地优先，更保护隐私**：导出包的解压、图片处理和文档转换都由本地助手在用户电脑上完成，不经过本项目的中转服务器；只有最终内容会按用户操作写入钉钉，临时文件会在任务结束时清理。
...
2. 第一次打开扩展时，按照提示下载并运行 `Notion2DingDing-Setup.exe`，安装本地助手。这样导出包的解压和转换会留在用户电脑上，不需要上传到本项目服务器。
